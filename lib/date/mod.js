...
* @param {Date} date - A date object
* */
export const formatDateToStandardString = (date, remove_timezone = true) => {
  let validated_date;
  if (remove_timezone) {
    const offset = date.getTimezoneOffset();
    validated_date = new Date(date.getTime() - (offset * 60 * 1000));
  } else {
    validated_date = date;
  }
...
/**
 * Receives a Date and returns a local formatted week
 * @param {Date} date
 * @returns {string} {Dia} de {Mes}
 * */
export const formatDateAsWeekLocal = (date) => {
  const standard_date = formatDateToStandardString(date);
  const day = standard_date.substr(8, 2);
  const month = months.get(standard_date.substr(5, 2));
